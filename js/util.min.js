...
var switchBtn=null;window.addEventListener('DOMContentLoaded',e=>{switchBtn=document.querySelectorAll('#switch_btn');!!switchBtn&&switchBtn.forEach(ele=>{ele.addEventListener('click',switchTheme);})},{once:true})})();;(function(){function changeSearchShow(e){if(!!searchContainer&&!!cover&&!!searchBtn&&(e.target==cover||e.target==searchBtn)){searchContainer.classList.toggle('show');cover.classList.toggle('show');searchInput.focus();e.stopPropagation();}}
var searchContainer=null;var cover=null;var searchBtn=null;var searchInput=null;window.addEventListener('DOMContentLoaded',e=>{searchContainer=document.querySelector('.search-container');searchBtn=document.querySelector('#search_btn');cover=document.querySelector('#cover');searchInput=document.querySelector('.search-input');!!searchBtn&&searchBtn.addEventListener('click',changeSearchShow);!!cover&&cover.addEventListener('click',changeSearchShow);document.onkeydown=function(event){var e=event||window.event||arguments.callee.caller.arguments[0];if(e.keyCode==75&&e.ctrlKey){e.preventDefault();searchContainer.classList.toggle('show');cover.classList.toggle('show');searchInput.focus();}}
docsearch({apiKey:'f7f9aeb8485880cfd0e747129ea4ddf9',indexName:'blogsearch',appId:'SGIZDO9IMB',inputSelector:'.docsearch-input',debug:true,algoliaOptions:{hitsPerPage:50},});},{once:true})})();;(function(){function scrollTo(targetHeight){if(!!singlePost&&!!header&&!!content&&!!footer){let totalHeight=content.offsetHeight+footer.offsetHeight+header.offsetHeight+160-document.body.clientHeight;if(targetHeight>totalHeight){targetHeight=totalHeight;}
...
clearActivedTocItem();setActiveTocItem(activeIndex);}}
function clearActivedTocItem(){let activeTocList=document.querySelectorAll('.toc li .active');!!activeTocList&&activeTocList.forEach(ele=>ele.classList.remove('active'));}
function setActiveTocItem(index){!!tocList[index]&&tocList[index].classList.add('active');tocContainerScroll();}
function tocContainerScroll(){let activeLi=document.querySelector('.toc li .active')?.parentNode;if(!!tocContainer){if(!!activeLi){if(activeLi.offsetTop+activeLi.clientHeight>tocContainer.clientHeight+tocContainer.scrollTop){tocContainer.scrollTop+=(activeLi.offsetTop+activeLi.clientHeight)
-
...
var singlePost=null;var header=null;var content=null;var footer=null;var headerList=null;var tocContainer=null;var tocList=null;var imgList=null;var menu=null;var backTopBtn=null;var percentage=null;window.addEventListener('DOMContentLoaded',e=>{singlePost=document.querySelector('.single-post');header=document.querySelector('.single-post .post .header');content=document.querySelector('.single-post .post .content');footer=document.querySelector('.single-post .post .footer');headerList=document.querySelectorAll('.content h1[id],h2[id],h3[id],h4[id],h5[id]');tocContainer=document.querySelector('.toc');tocList=document.querySelectorAll('.toc li .toc-link');imgList=document.querySelectorAll('.content img');menu=document.querySelector('.menu');backTopBtn=document.querySelector('#back_top_btn');percentage=document.querySelector('#percentage');if(!!imgList&&imgList.length>0){imgList.forEach(ele=>{ele.onerror=handleImgError;})}
